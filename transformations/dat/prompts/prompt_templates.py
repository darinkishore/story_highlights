...
def generate_category_plan_prompt(story, category_name, category_labels):
    ...
    prompt += "Please strive for accuracy (exact textual matching) and engagingness (variety, succinct excerpts) in your labels."
    prompt += "Please plan out exactly how these labeling rules could apply to the reddit story below. Your plan should comprise of 1-3 bullet points per example. It'll be your notes for later, so think carefully!\n\n"
    prompt += "# Labeling Rules for Reddit Stories\n\n"
    prompt += generate_label_section(category_name, category_labels)
    prompt += f"\n\n## Reddit Story\n```\n{story}\n```\n\n"
    prompt += "Please create a plan for labeling the story, for each (applicable) category. Do not label the story yet--make sure you feel confident that your plan will create an engaging video!"
    return prompt
...
def gen_all_category_plans(story):
    ...
    for category_name, category_labels in categories.items():
        prompts[category_name] = generate_category_plan_prompt(story, category_name, category_labels)
    return prompts
...
def generate_follow_up_prompt(category):
    ...
    prompt = "Thank you! Now, please proceed to label the reddit story, ensuring thoroughness and precision. Don't blindly highlight everything!\n\n"
    prompt += 'Please use the format `**Label**: "Specific excerpt"`. A well-done example is provided below: \n\n'
    prompt += "```\n"
    prompt += f"{example}\n"
    prompt += "```\n\n"
    prompt += "Engagingly and accurately labeling excerpts is crucial, as your output will be extracted via exact text matching, mapped to colors, then put into a TikTok video. Consider every line and maintain the formatting consistently. Thank you!"
    return prompt
